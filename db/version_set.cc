...
class VersionSet::Builder {
 private:
  ...
  /* ZcNote::这是set初始化的一种方式，自己定义类型的比较方式，而且必须是一个
             struct类型，重载()为何addfiles需要比较函数?因为addfiles需要排序 */
 ...
 public:
  ...
  void Apply(VersionEdit* edit) {
    ...
    for (VersionEdit::DeletedFileSet::const_iterator iter = del.begin();
	  ...
	  /*ZcNote:: _level是个LevelState类，里面放的是add和deletefile
	             是作为这个builder类自己的私有数据，apply起始就是
	             把edit里面的数据拷贝进了builder类的私有数据中。多次
	             apply就是合并处理 */
    ...
    for (size_t i = 0; i < edit->new_files_.size(); i++) {
      ...
      /* ZcNote::这句话有没有必要呢? 什么时候会有一个即将插入的file在delete列表中?即将插入的file
	             不都是新生成的么?*/
  ...
  void SaveTo(Version* v) {
    ...
    for (int level = 0; level < config::kNumLevels; level++) {
      ...
      /*ZcNote:: 这个base_是build类里面的，并不是外面的类带进来的，就是_current
	             这个version*/
      const std::vector<FileMetaData*>& base_files = base_->files_[level];
      std::vector<FileMetaData*>::const_iterator base_iter = base_files.begin();
      std::vector<FileMetaData*>::const_iterator base_end = base_files.end();
      const FileSet* added = levels_[level].added_files;
	 /* ZcNote::这里的files_都是初始化的一个空的，reserve是分配出这么多空间*/
      v->files_[level].reserve(base_files.size() + added->size());
      for (FileSet::const_iterator added_iter = added->begin();
           added_iter != added->end();
           ++added_iter) {
        // Add all smaller files listed in base_
        /* ZcNote:: 1、upper_bound 在begin和end之间利用cmp的方法
		           找到第一个大于*added_iter的元素，因此找到的第一个元素之前，都是
		           file里面最大的key都小于这个*added_iter的最小的key的。
		           2、那么先添加base的，然后添加added的，然后再添加剩余base的
		           不担心added里面有比base剩余部分大的么? 这个就是比较难懂的地方了
		           因为，在压缩的时候，是将level和要压缩到的level+1的所有file进行了比较
		           的，如果有重叠，就会一起进行压缩。因此，added这些file中，是肯定不会
		           发生和base中重叠的部分的。如果level+1中没有和level重叠的，那么added里面
		           的file也就一定不会大于base中剩余的部分，因为一开始就没有重叠
		           3、原来base中的file什么时候删除?就在MaybeAddFile中删除，当在deletefiles中
		           的时候就不添加，也就自然删除了 */
...
/* ZcNote::如果在delete文件中，那就什么都不做，否则添加到vector的最后面。因为
   这个vector是新new出来的一个，相当于两个排序链表进行再排序 */
...
VersionSet::VersionSet(const std::string& dbname,
  ...
  /* ZcNote:: 从这里可以看出：
              1、先有versionset，后有version，version是用
              versionset初始化的。而且一开始version中是什么都没有的，因此
              可以基本断定，version里面的内容都是由versionset恢复出来的 
              2、versionset在初始化的时候，也就是刚启动的时候，都是一个
              空的version链表，将current填进去。所以，current就是这个链表
              上最后的一个version元素 */
...
void VersionSet::AppendVersion(Version* v) {
  ...
  assert(v != current_);
  if (current_ != NULL) {
    /* ZcNote:: leveldb中所有的删除都是直接delete，删除父类的
                同时把包含类的内存也释放掉，这个逻辑在Unref中 */
	current_->Unref();
  }
  current_ = v;
  v->Ref();
...
/* ZcNote::需要delete的file就是inputs的两个数组，也就是两个level中选出来的重叠部分的所有file */
